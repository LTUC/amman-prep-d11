...
server.get('/authors/:id/books', viewAllBooks);
...
function addbooks(req, res) {
    const { title, isbn, imgurl, author_id } = req.body;
    const sql = 'INSERT INTO books(title,isbn,imgurl,author_id) values ($1,$2,$3,$4) RETURNING *;';
    const values = [title, isbn, imgurl, author_id];
    client.query(sql, values)
        .then((data) => {
            res.send(data.rows)
        })
